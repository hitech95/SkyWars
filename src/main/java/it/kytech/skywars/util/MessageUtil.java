	...
	public static String replaceVars(String msg, HashMap<String, String>vars){
		...
		for(String s:vars.keySet()){
			try{
				...
				SkyWars.$(Level.WARNING, "Failed to replace string vars. Error on "+s);
				error = true;
			}
		}
		if(error){
			SkyWars.$(Level.SEVERE, "Error replacing vars in message: "+msg);
			SkyWars.$(Level.SEVERE, "Vars: "+vars.toString());
			SkyWars.$(Level.SEVERE, "Vars Cache: "+varcache.toString());
		}
		return msg;
	}
	...
	public static String replaceVars(String msg, String[] vars){
		...
		for(String str: varcache.keySet()){
			try{
				...
				SkyWars.$(Level.WARNING,"Failed to replace string vars. Error on "+str);
				error = true;
			}
		}
		if(error){
			SkyWars.$(Level.SEVERE, "Error replacing vars in message: "+msg);
			SkyWars.$(Level.SEVERE, "Vars: "+Arrays.toString(vars));
			SkyWars.$(Level.SEVERE, "Vars Cache: "+varcache.toString());
		}
