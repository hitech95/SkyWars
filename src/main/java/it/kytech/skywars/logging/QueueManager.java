...
public class QueueManager {
	...
	public void setup(){
		baseDir = new File(SkyWars.getPluginDataFolder()+"/ArenaData/");
		try{
			if(!baseDir.exists()){
				baseDir.mkdirs();
			}
		...
		public void run(){
			...
			if(data != null){
				...
				if(a != -1){
					...
					SkyWars.$ ("Arena "+id+" reset. Rolled back "+(totalRollback+rb)+" blocks in "+iteration+" iterations ("+pt+" blocks per iteration Total time spent rolling back was "+time+"ms)");
					game.resetCallback();
				}
			}else{
				SkyWars.$ ("Arena "+id+" reset. Rolled back "+totalRollback+" blocks in "+iteration+" iterations. Total time spent rolling back was "+time+"ms");
				game.resetCallback();
			}
		}
